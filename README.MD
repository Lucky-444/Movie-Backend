...
currently whetehr we are a customer or client the user gets auto approved but we need to change the behaviour such that only customers get default approved a
and admins and clients are get pending status 

___JWT USES STEPS___
we are expecting the user to send email and password in the request body
1. we first verify if the email exists in the database or not
2. if email exists then we compare the password sent by the user with the encrypted password stored in the database using bcrypt library
3. if password matches then we create a JWT token using the user details and a secret key
4. we send this token to the user as response
5. the user stores this token in local storage or cookies
6. for every request to protected routes the user sends this token in the Authorization header
7. we verify this token using the secret key and if valid we grant access to the resource else we deny access
> ### A Unique Token is generated for each user upon successful authentication. This token encapsulates essential user information and is digitally signed to ensure its integrity and authenticity. The token typically includes:
- **User ID**: A unique identifier for the user.
- **Expiration Time**: A timestamp indicating when the token will expire, enhancing security by limiting its validity period.
- **Issued At**: A timestamp indicating when the token was issued.
- **User Role**: Information about the user's role (e.g., admin, user) to facilitate role-based access control.
- Any other relevant claims or metadata as needed by the application.
> ### This token is then sent to the client, which stores it (usually in local storage or cookies) and includes it in the Authorization header of subsequent requests to protected routes. The server verifies the token's signature and validity before granting access to the requested resources.###

__VERIFYING JWT TOKEN__
incase of JWT token we can send the token in request header
whenever we are trying to acess a protected resource
we need to send the token(which we recieve in Signin) in the authorization header as Bearer token
On receiving the request the server will verify the token using the secret key
if the token is valid then the server grants acess to the resource else it denies acess
//EX : Authorization: Bearer <token>
//EX : Authorization : ex-access-token-xyz <token>

__LOGOUT__
Two Ways to Handle Logout with JWT:
//By deleting the token on the client side (local storage or cookies || either token is expired or user manually delete it)


__RESET PASSWORD__
To implement a secure password reset functionality, we will follow these steps:
they need to provide their registered email address.and previously set password
otherwise not 
if previously set password got matched then we go forward 
and make the user to set new password

__UPDATE USER DETAILS__
we want to implement an endpoint that allows users to update their details .
becoz of fact that our users will be having userRoles and userStatus so we need to update those details as well

FOR IMPLEMENTING AUTHORIZATION we need to set up role based access control (RBAC) mechanism

__BOOKINGS__
setUp schema and model for Bookings with fields:
In order to setup bookings we need first understand the USecases
for a booking we need to associate the booking with a user and a movie
we can maintain status of the booking as well

The Booking schema will include the following fields: 
- Booking ID (unique identifier)
- User ID (foreign key referencing User)
- Movie ID (foreign key referencing Movie)
- Theater ID (foreign key referencing Theater)
- Showtime (date and time of the booking)
- Seats Booked (array of seat numbers)
- Total Price (calculated based on seat prices)
- Booking Status (confirmed, cancelled, pending)
- Timestamps (createdAt, updatedAt)

__PAYMENTS__ 
setUp schema and model for Payments with fields:
The Payment schema will include the following fields:
- Payment ID (unique identifier)
- Booking ID (foreign key referencing Booking)
- Amount (total payment amount)
- Status (payment status: pending, failed, success)


__APIS FOR BOOKINGS__
- POST /bookings - Create a new booking
         The Route will accept booking details in the request body and create a new booking record in the database. It will also handle seat availability checks and payment processing.
[inOrder to create booking the user must be authenticated]
         Route -> /mba/api/v1/bookings
         type -> POST
         request body -> {
            userId: ObjectId,
            movieId: ObjectId,
         ...
         response -> {
            data : {} // store all the data of booking
            bookingId: ObjectId,
            status: "confirmed",
            createdAt: Date
         }
- PUT /bookings/:id - Update a booking
         The Route will allow users to update their booking details, such as changing seats or showtime, before a certain cutoff time.
         Route -> /mba/api/v1/bookings/:id
         type -> PUT
         request body -> {
            seatsBooked: [String],
            showtime: Date
         ...
         response -> {
            data : {} // store all the updated data of booking
            bookingId: ObjectId,
            status: "updated",
            updatedAt: Date
         }
...
- GET /bookings - Get all bookings
         The Route will retrieve all bookings for the authenticated user, with optional filters for date range, status, etc.
         Route -> /mba/api/v1/bookings
         type -> GET
         query params -> {
...
__SHOW MODEL__
In Theatre Model we are just having list of movies that are being shown in that theatre
it does not have any information about the showtime of the movie in that theatre and available seats and timings for that particular movie in that theatre
what are the different showTime available for that movie in that theatre
what'S the Hall/Screen number where that movie is being shown in that theatre
//We have no idea about this information from the current Theater and Movie models

so we setUp a New Model Called [Show] Model
setUp schema and model for Shows with fields:
The Show schema will include the following fields:
- Show ID (unique identifier)
- Movie ID (foreign key referencing Movie)
- Theater ID (foreign key referencing Theater)
- Showtime (date and time of the show)
- Screen Number (the hall/screen number in the theater)
- Available Seats (array of available seat numbers)
- Price Per Seat (price for each seat in this show)
- Timestamps (createdAt, updatedAt)
/***in Theatre Model we have a property [shows] which is an array of ObjectIds referencing the Show model . We can Build in This Way also ***/

//Now for Payment Details and Booking associate with Show Model as well
__PAYMENT AND BOOKING ASSOCIATION__
user -> Booking -> show -> Movie -> payment
         |        -------------------------
         |                 Make Payment 
         |                             \
      waiting State [pendingState];     \
                                  \    |
                                   \   | 
                                    \  |
                                 Payment Model
                                 successFUll Payment 
                                 Get The Ticket
                                 convert the booking status to confirmed or successful

/***once the Booking is intiated the booking status will be in pending state or waiting state 
   until the payment is successfull this will remain for 5 mins if payment is not successfull within 5 mins then the booking will be cancelled automatically by the system and the seats will be released back to available seats pool***/
...
- POST /payments - Make a payment for a booking
         The Route will handle payment processing for a specific booking. It will create a payment record and update the booking status upon successful payment.
         [Route] -> /mba/api/v1/payments
         [type] -> POST
         request body -> {
            bookingId: ObjectId,
            amount: Number,
         ...
         response -> {
            data : {} // store all the payment data
            paymentId: ObjectId,
            status: "success",
            err : {},
...
__AXIOS__
Axios is a JavaScript library used to send HTTP requests (API calls) from your application to a server and handle the response easily.

Think of it as a better, cleaner alternative to fetch();
Send JSON data to backend
Handle responses & errors cleanly
Set headers (tokens, auth, etc.)
